...
- Learning the sim/distance between two objects/places can be useful to filter out duplicated cases and false posstive example, load visual-pairs and visual-pairs either run `sim.py` that glove with 840B tokens
 or run this fixed dic `M = containers.Map(pairsobject,cosine_sim)`   `M('airfield airliner'), M('crosswalk plaza'), etc `
